...
const formatMovementDate = function(date, locale) {
    const calcDaysPased = (date1, date2)=>Math.round(Math.abs((date2 - date1) / 86400000));
...
const displayMovements = function(acc, sort = false) {
    ...
    const movs = sort ? acc.movements.slice().sort((a, b)=>a - b) //*Sort afecta el array original
     : acc.movements;
...
const calcDaysPased = (date1, date2)=>Math.abs((date2 - date1) / 86400000); //*Para convertirlo a t unix, se divide entre 1000mx, 60seg, 60min, 24hrs
...
const message = `${(today - pasado) / 86400000} dias han pasado`;
// console.log(message);
...
welcome.addEventListener('keydown', function(e) {
    ...
    if (e.key) welcome.style.backgroundColor = colors[e.key];
});
//*Acepta [[],[]]
...
const pizzaTimer = setTimeout((ing1, ing2 // console.log(`Heres your pizza with ${ing1} and ${ing2}`),
)=>3000, ...ingredients);
...
//# sourceMappingURL=timerBankistApp.44983732.js.map
